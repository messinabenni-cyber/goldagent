...
def diagnose_call_result(
    ...
    *,
    ...
    if code is None:
        ...
        lines.append("")
        lines.append("  AUTO-APPLIED FIXES (tool attempted these automatically):")
        lines.append("  ✓ TCP transport fallback          [AUTO-FIX B — tried]")
        lines.append("  ✓ Alt SIP port 5080               [AUTO-FIX C — tried]")
        lines.append("  ✓ STUN public IP for Contact/Via  [AUTO-FIX A — tried if NAT detected]")
        lines.append("")
        lines.append("  MANUAL FIXES (require network access):")
        lines.append("  → SSH/VPN tunnel into target network (guarantees same-subnet view).")
        lines.append("  → Disable SIP ALG on all routers between scanner and PBX.")
        lines.append("  → Add --stun auto if auto-STUN was not triggered automatically.")
    else:
